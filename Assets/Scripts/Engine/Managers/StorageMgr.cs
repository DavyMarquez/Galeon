...
public abstract class StorageMgr
{
    ...
    public T Get<T>(string section, string key)
	{
        ...
        Debug.Assert(m_serialized.ContainsKey(section), "La seccion " + section + " no existe.");
        StorageKey sk = m_serialized[section];
        Debug.Assert(sk.ContainsKey(key), "La clave " + key + " no existe en seccion " + section + ".");
        return sk.Get<T>(key);
	}
    ...
    protected void ProcessAttributes()
	{
		...
		for(int i = 0; i < attributes.Length; ++i)
		{
			if(attributes[i].GetType() == typeof (AllowedTypeToStorage))
			{
                ...
                AllowedTypeToStorage aux = attributes[i] as AllowedTypeToStorage;
                m_allowedTypes.Add(aux.m_type);
                count++;
