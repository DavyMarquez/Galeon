...
public class StorageKey 
{
	...
	{
        Debug.Assert(m_data.ContainsKey(key),"the key "+key+" is not exist");
		return m_data[key].ToString();
	}
	
	public System.Type GetContentType(string key)
	{
        Debug.Assert(m_data.ContainsKey(key),key+" Is not exists");
	...
	{
        Debug.Assert(AllowedType(typeof(T)),typeof(T)+" Is not allowd");
		bool exist = m_data.ContainsKey(key);
        Debug.Assert(exist,key+" Is not exists");
        T obj = (T)m_data[key];
        return obj;
	}
